...
// cinatra-ai/cinatra#1107; lands with the connector-setup rebuild, this repo
// issue #38 + #39). The "use server" actions in ./actions redirect back to the
// connector setup route carrying an outcome CODE (`?notice=<code>` /
// `?error=<code>`); the <SearchParamToast> island mounted in ./settings-page
// maps each code to a STATIC, server-trusted message here — it NEVER toasts
// URL-derived text (a crafted `?error=<spoofed link>` maps to no entry and is
// ignored). This module is the single source of truth so the action emitters
// and the mount-site message map cannot drift.
// -----------------------------------------------------------------------------
...
export const GITHUB_NOTICE_MESSAGES = {
  ...
  disconnected: "GitHub disconnected. The connector will stop working until you connect it again.",
} as const;
...
export const GITHUB_ERROR_MESSAGES = {
  ...
  "repo-save-failed": "Could not save the GitHub repository selection. Try again.",
  "disconnect-failed": "Could not disconnect GitHub. Try again.",
} as const;
...
// One <SearchParamToast> config entry per code, mounted in ./settings-page. The
